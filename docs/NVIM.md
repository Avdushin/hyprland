...
Плавающий терминал использует один постоянный буфер. При скрытии окно закрывается, но shell-процесс продолжает работать, поэтому текущий каталог, запущенные команды и вывод сохраняются.
